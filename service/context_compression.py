...
class ContextCompressionService:
    ...
    上下文压缩服务，实现五步压缩策略
    """
    def __init__(self, vector_store: VectorStore, embeddings: BaseEmbeddings, llm):
    ...
    def compress(self, query: str, context: List[Dict[str, Any]], max_tokens: int = 1000) -> str:
        """
        压缩上下文
        """
        # 第一步：大结果存磁盘
        ...
        for item in context:
            if isinstance(item, dict) and 'content' in item:
                ...
                if content_size > 50 * 1024:  # 50KB
                    ...
                    total_size += len(processed_item['content'].encode('utf-8'))
                else:
        ...
        while total_size > 200 * 1024:
            ...
            for i, item in enumerate(processed_context):
                if item == largest_item:
                    ...
                    total_size += len(processed_context[i]['content'].encode('utf-8'))
                    break
        ...
        for item in processed_context:
            if 'timestamp' in item:
                ...
                if age < 3600:  # 1小时内的消息保留
                    ...
                    snip_tokens_freed += self._calculate_token_count(item['content'])
            else:
                recent_context.append(item)
        
        # 第三步：裁剪老的工具输出
        for item in recent_context:
            if 'tool_name' in item and self._is_retrievable_tool(item['tool_name']):
                if 'timestamp' in item:
                    age = current_time - item['timestamp']
    ...
    def add_context(self, context: List[Dict[str, Any]]) -> None:
        ...
        # 向量化存储
        documents = [Document(page_content=item['content']) for item in context]
