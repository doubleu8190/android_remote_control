...
class PortAllocator:
    ...
    def __init__(self):
        ...
        self.base_port = int(
            os.getenv(
                "SCRCPY_WS_PORT_BASE",
                self.DEFAULT_BASE_PORT))
        self.port_range = int(
            os.getenv(
                "SCRCPY_WS_PORT_RANGE",
                self.DEFAULT_PORT_RANGE))
        self.max_retries = int(
            os.getenv(
                "SCRCPY_WS_MAX_RETRIES",
                self.DEFAULT_MAX_RETRIES))

        # 状态目录用于跨进程同步
        self.state_dir = Path(
            os.getenv(
                "SCRCPY_PORT_STATE_DIR",
        ...
        logger.info(
            f"PortAllocator 初始化完成，端口范围: {self.base_port}-{self.base_port + self.port_range - 1}")
    ...
    def allocate_port(self, session_id: Optional[str] = None,
        ...
        for offset in range(self.max_retries):
            ...
            if self._check_and_reserve_port(port, session_id):
                logger.info(
                    f"成功分配端口: {port} (尝试 {offset + 1}/{self.max_retries})")
                return port

        # 如果在基础范围内未找到，尝试随机端口
        logger.warning("在基础范围内未找到可用端口，尝试扩展搜索")
        for attempt in range(self.max_retries):
            # 尝试随机端口（在基础端口附近）
            # nosec B311: 端口分配不需要密码学安全的随机数
    ...
    def _check_and_reserve_port(
            self, port: int, session_id: Optional[str]) -> bool:
        """检查并预留端口（原子操作）
    ...
    def _is_port_actually_used(
            self, port: int, port_info: dict[str, Any]) -> bool:
        """检查端口是否实际被占用
        ...
        try:
            ...
            logger.debug(f"检查端口 {port} 使用状态时出错: {e}")
            return False
    ...
    def release_port(self, port: int):
        ...
        if hasattr(self, '_port_sockets') and port in self._port_sockets:
            ...
            try:
                ...
                logger.debug(f"关闭端口 {port} 的socket时出错: {e}")
